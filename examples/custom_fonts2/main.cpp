...
typedef void (*CSprite2d_DrawTxRectFn)(CRect const&, CRGBA const&);
static CSprite2d_DrawTxRectFn CSprite2d_DrawTxRect = nullptr;
            ...
            for(int x=0;x<w;x++)
            {
                ...
                if(c > 0)
                {
                    ...
                    if(shadowColor && shadowOffset > 0.0f)
                    {
                        ...
                        CSprite2d_DrawTxRect(shadowRect, scol);
                    }

                    // --- Draw main glyph ---
                    CRect rect(px, py, px+1, py+1);
                    CRGBA col(color.r, color.g, color.b, c);
                    CSprite2d_DrawTxRect(rect, col);
                }
            }
        }
...
DECL_HOOKv(CHud__Draw)
{
    ...
    if(g_fontLoaded && CSprite2d_DrawTxRect)
    {
        CRGBA mainColor(0,255,0,255);
        CRGBA shadowCol(0,0,0,255);
        DrawString("MEGA Mind # / ! $ % ^ & * ( ) _ - + = [ ] { } | \\ < > : ; ' ~ ` @ . , ? 0123456789", 180.0f, 100.0f, 24.0f, mainColor);
		
 /* Example 1: green text with black shadow (2px offset)
 DrawString("MEGA Mind # / ! $ % ^ & * ( ) _ - + = [ ] { } | \\ < > : ; ' ~ ` @ . , ? 0123456789", 180.0f, 100.0f, 24.0f, mainColor, &shadowCol, 1.0f);
 */
...
extern "C" void OnModLoad()
{
    ...
    if(pGTAVC && hGTAVC)
    {
        ...
        CSprite2d_DrawTxRect = (CSprite2d_DrawTxRectFn)aml->GetSym(hGTAVC, "_ZN9CSprite2d10DrawTxRectERK5CRectRK5CRGBA");

        if(CSprite2d_DrawTxRect) logi("CSprite2d::DrawTxRect resolved");
        else loge("Failed to resolve CSprite2d::DrawTxRect");
    }
